...
def modified_secant(f, a, b, tol=0.01):
    if f(a) * f(b) >= 0:
        raise ValueError("Modified Secant method fails. Choose different a and b.")
...
def newton_raphson(f, df, a, tol=0.01, max_i=50):
    table = []
    for i in range(max_i):
        ...
        b = a - f_a / df_a
        table.append([i+1, round(a, 4), round(f_a, 4), round(df_a, 4), abs(f_a), i+1 == max_i])
...
def fixed_point(g, f, a, N, tol=0.01):
    g = lambda x: f(x) + x  # Define g(x) = f(x) + x
    table = []
    for i in range(N):
        b = g(a)
        table.append([i+1, round(a, 4), round(f(a), 4), round(g(a), 4), abs(f(a)), i+1 == N])
...
def display_tables(titles, solutions, dfs):
    ...
    if num_tables == 1:
        axes = [axes]  # Ensure axes is iable if there's only one table

    plt.subplots_adjust(hspace=1.5, wspace=1.5)  # Add space between text and table
...
def solution():
    ...
    methods = [
        ...
        ("Fixed Point Method", fixed_point(g, f, 1, 5))
    ]
